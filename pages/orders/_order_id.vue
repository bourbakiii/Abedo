  ...
  <div class="page order-page wrapper">
    <div class="order-page__content content">
      <Breadcrumbs class="order-page__content__breadcrumbsad adaptive-non"
                   :way="[{name:'История заказов', link:`/orders`}, {name:`Заказ №${order.id}`, link:`/orders/${order.id}`}]"/>
      ...
      <div class="order-page__content__least">
        ...
        <p v-if='order.price_with_discount<order.price' class="order-page__content__least__price_full">
          {{ order.price }}₽</p>
      </div>
      <button
...
export default {
  ...
  async asyncData({$axios, route, error, store}) {
    let to_return_order = {},
      loading = true;
    await $axios
      .$get(`${$axios.defaults.baseURL}/api/order/${route.params.order_id}}`, {
        headers: {
