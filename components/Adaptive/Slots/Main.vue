<template>
  <div class="navigation-slot navigation-slot-main">
    <NuxtLink to='/' class="navigation-slot-main__logo">
      <img
        src="@/assets/images/abedo-logo.png"
        class="navigation-slot-main__logo__image"
      />
      <img
        src="@/assets/images/abedo-logo-text-adaptive.png"
        class="navigation-slot-main__logo__text"
      />
    </NuxtLink>
    <svg
    @click='$router.push("/profile")'
      class="navigation-slot-main__icon"
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <circle cx="12" cy="10" r="3" fill="#5C6784" />
      <circle cx="12" cy="12" r="9" stroke="#5C6784" stroke-width="1.2" />
      <path
        d="M17.8719 18.8083C17.9489 18.7468 17.9799 18.6436 17.9452 18.5513C17.5693 17.5518 16.8134 16.6706 15.7814 16.0332C14.6966 15.3632 13.3674 15 12 15C10.6326 15 9.30341 15.3632 8.21858 16.0332C7.18663 16.6706 6.43066 17.5518 6.05477 18.5513C6.02009 18.6436 6.05115 18.7468 6.12813 18.8083C9.56196 21.552 14.438 21.552 17.8719 18.8083Z"
        fill="#5C6784"
        stroke="#5C6784"
        stroke-width="1.2"
        stroke-linecap="round"
      />
    </svg>
  </div>
</template>
<style lang="scss" scoped>
.navigation-slot-main {
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: space-between;
  flex-direction: row;
  &__logo {
    display: flex;
    align-items: center;
    justify-content: center;
    flex-direction: row;
    &__image {
      width: 30px;
      height: 30px;
      margin-right: 10px;
    }
    &__text {
      font-family: "Montserrat";
      font-style: normal;
      font-weight: 500;
      font-size: 20px;
      line-height: 24px;
      color: $darkblue;
    }
  }
}
</style>